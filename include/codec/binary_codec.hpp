...
namespace codec
{
    namespace binary
    {
        ...
        template <class Codec,
                  class Object,
                  typename std::enable_if<std::is_base_of<Binary, Codec>::value,
                                          int>::type = 0>
        void register_meta(Codec& codec, Object& object, Prefix_Type&& meta)
        {
        ...
        void field(Decode& c, uint8_t& value) { value = c.data[c.index++]; }

        //
        // uint16_t
        //
        void field(Encode& c, uint16_t& value)
        {
            c.data.push_back((value >> 0) & 0xFF);
            c.data.push_back((value >> 8) & 0xFF);
        }

        void field(Decode& c, uint16_t& value)
        {
        ...
        Meta get_meta(Binary& c, Object& object, Meta default_meta)
        {
                       ...
                       binary::Prefix_Type::L8})
        {
            ...
            value = std::string(c.data.begin() + c.index,
                                c.data.begin() + c.index + l);
            c.index += l;
        }
